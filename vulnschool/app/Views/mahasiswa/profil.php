...
<div class="d-flex justify-content-between align-items-center mb-4">
    <div>
        <h4 class="fw-bold mb-1">Profil Mahasiswa</h4>
        <p class="text-muted mb-0" style="font-size:.85rem;">
            <?= esc($mahasiswa['nim']) ?> — <?= esc($mahasiswa['nama']) ?>
        </p>
    </div>
    <?php
    $mhsLogin = (new \App\Models\MahasiswaModel())->findByUserId(session()->get('user_id'));
    $isOwner = $mhsLogin && (int)$mhsLogin['id'] === (int)$mahasiswa['id'];
    ?>
    <?php if ($isOwner): ?>
        <a href="/mahasiswa/profil/edit" class="btn btn-primary btn-sm px-3">
            ...
            <div class="card-body text-center py-4">
                <?php if (!empty($mahasiswa['foto'])): ?>
                    <img src="/uploads/foto/<?= esc($mahasiswa['foto']) ?>" alt="Foto"
                         class="rounded-circle mb-3" style="width:100px;height:100px;object-fit:cover;border:3px solid var(--vs-border);">
                <?php else: ?>
